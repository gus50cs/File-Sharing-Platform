...
func (s *SmartContract) CreateDocument(ctx contractapi.TransactionContextInterface, DocumentID string, DocumentCID string, Owner string, access string, Reupload string) error{
	exists, err := s.AssetExists(ctx, DocumentID, Owner, DocumentCID)
	if err != nil {
		return err
	}
	if exists {
	...
	updatedTime := currentTime.Add(3 * time.Hour)
	updatedTimeString := updatedTime.Format("15:04:05 02-01-2006")
	asset := LNGDocument{
		DocumentID:		DocumentID,
		DocumentCID:    DocumentCID,
		Owner:          Owner,
		Reupload: 		Reupload,
		Timestamp:		updatedTimeString,
	}
...
func (s *SmartContract) UpdateAccess(ctx contractapi.TransactionContextInterface, access string, DocumentID string, Owner string, DocumentCID string) error {
	asset, err := s.ReturnAsset(ctx, DocumentID, Owner, DocumentCID)
	if err != nil {
		return err
	}
...
func (s *SmartContract) DeleteAsset(ctx contractapi.TransactionContextInterface, DocumentID string, Owner string, DocumentCID string) error {
	qrcode, err := s.GetQR(ctx, DocumentID, Owner, DocumentCID)
	if err != nil {
...
func (s *SmartContract) AssetExists(ctx contractapi.TransactionContextInterface, DocumentID string, Owner string, DocumentCID string) (bool, error) {
	qrcode, err := s.GetQR(ctx, DocumentID, Owner, DocumentCID)
	if err != nil {
...
func (s *SmartContract) ReturnAsset(ctx contractapi.TransactionContextInterface, DocumentID string, Owner string, DocumentCID string) (LNGDocument, error) {
	qrcode, err := s.GetQR(ctx, DocumentID, Owner, DocumentCID)
	if err != nil {
...
func (s *SmartContract) GetQR(ctx contractapi.TransactionContextInterface, value1 string, value2 string, value3 string) (string, error) {
	...
	for resultsIterator.HasNext() {
		...
		if asset.DocumentID == value1 && asset.Owner == value2 && asset.DocumentCID == value3{
			return asset.QR, nil
		}
	}
