...
const (
	mspID        = "org2MSP"
	cryptoPath   = "/home/kevin/thesis/organizations/org2"
	certPath     = cryptoPath + "/users/user/msp/signcerts/cert.pem"
	keyPath      = cryptoPath + "/users/user/msp/keystore/"
	tlsCertPath  = cryptoPath + "/peers/peernumber1/tls-msp/tlscacerts/ca.crt"
	peerEndpoint = "localhost:2012"
	gatewayPeer  = "peernumber1.org2"
)
...
func main() {
	...
	r.GET("/update-access", func(c *gin.Context) {
		...
		for i := range reuploaded {
			sortTimestampsAndNames(&reuploaded[i])
		}
	...
	r.POST("/update-access", func(c *gin.Context) {
		...
		fmt.Println(documentID, owner, accessListStr, documentCID)
		// Update the access list for the documentID and owner
		//smartfunc.UpdateAccess(contract, accessListStr, documentID, owner, documentCID)
	...
	r.POST("/delete", func(c *gin.Context) {
		...
		owner := c.PostForm("ownerID")
		documentCID := c.PostForm("documentCID")
		fmt.Println(documentID, owner, documentCID)
	...
	for _, asset := range uniqueAssets {
		...
		c.HTML(http.StatusOK, "Reuploaded.html", gin.H{
			"Assets": filteredUnreuploadedAssets,
...
func sortTimestampsAndNames(doc *AssetOwner) {
