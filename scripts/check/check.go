...
func Insert (userID string, Timestamp string, IsChecked bool, IsReuploaded bool) {
	...
	if exists {	
		...
		_, err := db.Exec(`UPDATE checkbox SET IsChecked = ? WHERE UserID = ? AND Timestamp  = ? AND IsReuploaded = ?`,
			IsChecked, userID, Timestamp, IsReuploaded)
		if err != nil {
		...
		_, err = db.Exec(`INSERT INTO checkbox (UserID, Timestamp, IsChecked, IsReuploaded) VALUES (?, ?, ?, ?)`, userID, Timestamp, IsChecked, IsReuploaded)
...
func RetrieveChecked (userID string) []*Document {
	...
	for rows.Next() {
		var doc Document
		err := rows.Scan(&doc.UserID, &doc.Timestamp, &doc.IsChecked, &doc.IsReuploaded)
		if err != nil {
...
func RetrieveUploaded (userID string) []*Document {
	...
	// Query the checked files for a specific user
	query := "SELECT * FROM checkbox WHERE UserID = ? AND IsReuploaded = ?"
	rows, err := db.Query(query, userID, true)
	if err != nil {
		fmt.Println("Failed to retrieve data from checkbox table:", err)
		return nil
...
func GetUsernames(username string) ([]string, error) {
